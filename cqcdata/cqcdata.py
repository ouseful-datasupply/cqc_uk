...
def _get_active_locations(con, exists='replace'):
    ...
    d2[['Location ID', 'Type', 'Measure', 'value' ]].to_sql(con=con, name='active_location_measures',
...
def _get_ratings(con, exists='replace'):
    ...
    tmpcols = ['Service / Population Group', 'Latest Rating', 'Publication Date','Report Type']
    basecols = [c for c in ratings_locations.columns if c not in tmpcols]
    ratings_locations[basecols].drop_duplicates().to_sql(con=con, name='ratings_locations',
                                                         if_exists=exists, index=False)
    ratings_locations[['Location ID']+tmpcols].to_sql(con=con, name='rated_locations',
                                                      if_exists=exists, index=False)
    ...
    ratings_providers[basecols].drop_duplicates().to_sql(con=con, name='ratings_providers',
                                                         if_exists=exists, index=False)
    ratings_providers[['Provider ID']+tmpcols].to_sql(con=con, name='rated_providers',
                                                      if_exists=exists, index=False)
...
def _create_index(con, table, cols, idx=None, unique=False):
    ...
    q = '''CREATE {unique} INDEX "{idx}" ON "{table}" ({cols});'''.format(table=table, idx=idx,
